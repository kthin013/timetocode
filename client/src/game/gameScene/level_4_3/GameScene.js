...
class GameScene extends Phaser.Scene {
   ...
   * @param {ItemSpriteObject} orangeObject2
   */
  constructor(playerObject, orangeObject, orangeObject2) {
    super("gameScene");
    this.playerObject = playerObject;
    this.orangeObject = orangeObject;
    this.orangeObject2 = orangeObject2;
  }
  ...
  createAnims() {
    this.start = this.physics.add.sprite(GAMECONSTANT.LEVEL_4_3_START.x + GAMECONSTANT.SPRITE_SIZE / 2, GAMECONSTANT.LEVEL_4_3_START.y + GAMECONSTANT.SPRITE_SIZE / 4, "start_anim");
    createStartAnims(this.anims);
    this.start.play(`start_idle`);

    this.end = this.physics.add.sprite(GAMECONSTANT.LEVEL_4_3_END.x + GAMECONSTANT.SPRITE_SIZE / 2, GAMECONSTANT.LEVEL_4_3_END.y + GAMECONSTANT.SPRITE_SIZE / 4, "end_anim");
    ...
    this.orange2 = this.physics.add.sprite(this.orangeObject2.startPos.x, this.orangeObject2.startPos.y, "orange_anim");
    createOrangeAnims(this.anims);
    this.orange2.play(`orange_idle`);
  ...
  endCheck() {
    ...
    // _.map([this.orangeObject, this.orangeObject2], (item) => {
    //   isAllItemInPos = isAllItemInPos && item.getIsInPosition()
    // });
    if ((this.orangeObject.getIsInPosition() && this.orangeObject2.getIsInPosition()) ||
      (this.orangeObject.currentPos.x === this.orangeObject2.endPos.x &&
        this.orangeObject.currentPos.y === this.orangeObject2.endPos.y &&
        this.orangeObject2.currentPos.x === this.orangeObject.endPos.x &&
        this.orangeObject2.currentPos.y === this.orangeObject.endPos.y)
    ) {
      isAllItemInPos = true;
    } else isAllItemInPos = false;
    (
      this.player.x >= GAMECONSTANT.LEVEL_4_3_END.x &&
      this.player.y >= GAMECONSTANT.LEVEL_4_3_END.y &&
      this.player.x < GAMECONSTANT.LEVEL_4_3_END.x + GAMECONSTANT.SPRITE_SIZE &&
      this.player.y < GAMECONSTANT.LEVEL_4_3_END.y + GAMECONSTANT.SPRITE_SIZE && isAllItemInPos
  ...
  handleItemsTakeAndDrop() {
    if ((this.orangeObject.currentPos.x !== this.orangeObject2.currentPos.x) ||
      (this.orangeObject.currentPos.y !== this.orangeObject2.currentPos.y)) {
  ...
  handleIsOnPosition() {
    if (
      ...
      (this.player.x === (512 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (320 + GAMECONSTANT.SPRITE_SIZE / 2)) ||
      (this.player.x === (256 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (320 + GAMECONSTANT.SPRITE_SIZE / 2))
    ) {
      this.playerObject.isOn = "blue";
    } else if (
      (this.player.x === (576 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (320 + GAMECONSTANT.SPRITE_SIZE / 2)) ||
      (this.player.x === (192 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (320 + GAMECONSTANT.SPRITE_SIZE / 2))
    ) {
      this.playerObject.isOn = "red";
    }
    else if (
      (this.player.x === (384 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (192 + GAMECONSTANT.SPRITE_SIZE / 2)) ||
      (this.player.x === (512 + GAMECONSTANT.SPRITE_SIZE / 2) && this.player.y === (192 + GAMECONSTANT.SPRITE_SIZE / 2)) ||
