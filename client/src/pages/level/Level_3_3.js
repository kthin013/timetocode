...
import Preloader from "../../game/gameScene/level_3_3/Preloader";
import GameScene from "../../game/gameScene/level_3_3/GameScene";
...
const Level_3_3 = () => {
  ...
  const [playerObject, setPlayerObject] = useState(new SpriteObject(
    GAMECONSTANT.LEVEL_3_3_START.x + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_3_3_START.y + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_3_3_FACING, "3_3", gameState, setGameState
  ));
  const [bananaObject, setBananaObject] = useState(new ItemSpriteObject(
    { x: 576 + GAMECONSTANT.SPRITE_SIZE / 2, y: 384 + GAMECONSTANT.SPRITE_SIZE / 2 },
    { x: 576 + GAMECONSTANT.SPRITE_SIZE / 2, y: 384 + GAMECONSTANT.SPRITE_SIZE / 2 },
    { x: 576 + GAMECONSTANT.SPRITE_SIZE / 2, y: 384 + GAMECONSTANT.SPRITE_SIZE / 2 },
    "banana"
  ));
  ...
  useEffect(() => {
    ...
    blocklyWorkspaceRef.current = Blockly.inject("blocklyDiv", {
      toolbox: BlocklyConfig.TOOLBOX_LEVEL_3_3,
      ...BlocklyConfig.WORKSPACE,
      maxInstances: {
        "movement": 2, "take": 1, "controls_whileUntil": 1, "controls_if": 1, "isHold": 1, "isOn": 1
      }
    });
