...
import Preloader from "../../game/gameScene/level_4_3/Preloader";
import GameScene from "../../game/gameScene/level_4_3/GameScene";
...
const Level_4_3 = () => {
  ...
  const [playerObject, setPlayerObject] = useState(new SpriteObject(
    GAMECONSTANT.LEVEL_4_3_START.x + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_4_3_START.y + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_4_3_FACING, "4_3", gameState, setGameState
  ));
  const [orangeObject, setOrangeObject] = useState(new ItemSpriteObject(
    { x: 512 + GAMECONSTANT.SPRITE_SIZE / 2, y: 256 + GAMECONSTANT.SPRITE_SIZE / 2 },
    { x: 576 + GAMECONSTANT.SPRITE_SIZE / 2, y: 320 + GAMECONSTANT.SPRITE_SIZE / 2 },
    { x: 512 + GAMECONSTANT.SPRITE_SIZE / 2, y: 256 + GAMECONSTANT.SPRITE_SIZE / 2 },
    "orange"
  ));
  const [orangeObject2, setOrangeObject2] = useState(new ItemSpriteObject(
    { x: 256 + GAMECONSTANT.SPRITE_SIZE / 2, y: 384 + GAMECONSTANT.SPRITE_SIZE / 2 },
  ...
  useEffect(() => {
    ...
    phaserGameRef.current = new Phaser.Game({
      ...
      scene: [Preloader, new GameScene(playerObject, orangeObject, orangeObject2)]
    });
    blocklyWorkspaceRef.current = Blockly.inject("blocklyDiv", {
      toolbox: BlocklyConfig.TOOLBOX_LEVEL_4_3,
      ...BlocklyConfig.WORKSPACE,
      maxInstances: { "movement": 5 }
    });
