...
import Preloader from "../../game/gameScene/level_2_8/Preloader";
import GameScene from "../../game/gameScene/level_2_8/GameScene";
...
const Level_2_8 = () => {
  ...
  const [playerObject, setPlayerObject] = useState(new SpriteObject(
    GAMECONSTANT.LEVEL_2_8_START.x + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_2_8_START.y + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_2_8_FACING, "2_8", gameState, setGameState
  ));
  ...
  useEffect(() => {
    ...
    blocklyWorkspaceRef.current = Blockly.inject("blocklyDiv", {
      toolbox: BlocklyConfig.TOOLBOX_LEVEL_2_8,
      ...BlocklyConfig.WORKSPACE,
      maxInstances: { "controls_repeat_ext": 1, "num": 1, "movement": 5, "turn": 2, "controls_if": 2, "isOn": 2 }
    });
