...
import Preloader from "../../game/gameScene/level_3_7/Preloader";
import GameScene from "../../game/gameScene/level_3_7/GameScene";
...
const Level_3_7 = () => {
  ...
  const [playerObject, setPlayerObject] = useState(new SpriteObject(
    GAMECONSTANT.LEVEL_3_7_START.x + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_3_7_START.y + GAMECONSTANT.SPRITE_SIZE / 2,
    GAMECONSTANT.LEVEL_3_7_FACING, "3_7", gameState, setGameState
  ));
    ...
    if (gameState === "End") {
      setTimeout(() => {
        ...
        if (playerObject.isOnGoal) {
          setWinOrLose("Win");
          if ((Date.now() - timer) < 900000) {
            setStarNum(3);
          } else if ((Date.now() - timer) < 1200000) {
            setStarNum(2);
          } else {
            setStarNum(1);
          }
  ...
  useEffect(() => {
    ...
    blocklyWorkspaceRef.current = Blockly.inject("blocklyDiv", {
      toolbox: BlocklyConfig.TOOLBOX_LEVEL_3_7,
